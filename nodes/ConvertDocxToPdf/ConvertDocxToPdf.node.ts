...
import { description, execute } from './actions/docxToPdf';

export class ConvertDocxToPdf implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				await (execute as Function).call(this, i, returnData, 'docxToPdf');
			} catch (error) {
